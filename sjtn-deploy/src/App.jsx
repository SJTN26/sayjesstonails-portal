...
const AuthPortal = ({ onLogin, onBack, onBook }) => {
 ...
 // Mark referral converted if this login followed a referral link
 try {
 const refId = sessionStorage.getItem('sjtn_ref_id');
 if (refId) {
 supabase.functions.invoke('track-referral', { body: { action: 'mark_converted', referral_id: refId } });
 sessionStorage.removeItem('sjtn_ref_id');
 sessionStorage.removeItem('sjtn_ref');
 }
 } catch {}
...
.then(() => {
 ...
 <div style={{ fontSize: 10, fontWeight: 700, color: B.blushLight, letterSpacing: 2, textTransform: "uppercase", marginBottom: 8 }}>Program Complete GRAD</div>
 ...
 As a program graduate, you keep your community access for 1 full year — no monthly fee. That's my gift to you for doing the work. GRAD
 </p>
 ...
 "GRAD Graduate badge on your community posts",
 ...
 <div style={{ fontSize:9, fontWeight:700, color:B.blush, letterSpacing:1.5, textTransform:"uppercase", marginBottom:4 }}>Session Prep Submitted Done</div>
...
const GradWelcomeModal = ({ user, onDismiss, isMobile, isWelcomeBack = false }) => (
 ...
 <div style={{ background:B.black, padding:"24px 32px", borderLeft:`4px solid ${B.blush}` }}>
 <div style={{ fontSize:10, fontWeight:700, color:B.blushLight, letterSpacing:2, textTransform:"uppercase", marginBottom:8 }}>Welcome Back, Graduate GRAD</div>
 ...
 {[
 { icon:"GRAD", text:"Graduate badge next to your name on every post — the community knows you went through the full program" },
...
.then(({ data }) => {
 ...
 return (
 <div style={{ display: "flex", height: "100dvh", overflow: "hidden", fontFamily: FONTS.body, background: B.off, position: "relative" }}>
 <style>{`@import url('https://fonts.googleapis.com/css2?family=Barlow+Condensed:ital,wght@0,300;0,700;0,900;1,300;1,700;1,900&family=DM+Sans:wght@300;400;500;600&display=swap'); *,*::before,*::after{box-sizing:border-box;margin:0;padding:0} button{-webkit-tap-highlight-color:transparent;transition:opacity.15s;cursor:pointer} button:active{opacity:.78} input,textarea{font-size:16px!important;font-family:inherit} ::-webkit-scrollbar{width:3px} ::-webkit-scrollbar-thumb{background:${B.cloud}}`}</style>
...
const ApplicationsView = () => {
 ...
 {app.status === "pending" && (
 <div style={{ display:"flex", gap:8 }}>
 <button onClick={() => approve(app.id)} style={{ padding:"9px 20px", background:B.success, border:"none", color:B.white, fontSize:10, fontWeight:700, cursor:"pointer", fontFamily:FONTS.body, letterSpacing:1, textTransform:"uppercase" }}>Done Approve — Start Trial</button>
 ...
 {app.status === "approved" && app.paid && (
 <div style={{ fontSize:12, color:B.success, fontWeight:700 }}>Done Active paying member since {app.trialStart}</div>
 )}
 </div>
 )}
 </div>
...
const InviteGraduateForm = () => {
 ...
 {done ? "Done Invited!" : busy ? "Sending…" : "GRAD Invite as Graduate"}
 </button>
 </div>
 <div style={{ fontSize:10, color:"#2D7D4E", marginTop:8, fontWeight:300 }}>They'll receive a community invite email and get the GRAD Graduate badge on their posts.</div>
 </div>
 );
};
...
.then(({ data }) => { if (data?.voice) setJessVoiceAdmin(data.voice); });
 ...
 // ── Admin Community state moved to standalone AdminCommunity component ──
  ...
  return (
    <div style={{ padding: isMobile ? "20px 18px 100px" : "28px 32px", maxWidth: 1020, width: "100%" }}>
      <div style={{ marginBottom: 20 }}>
        <p style={{ fontSize:9, fontWeight:700, color:B.blush, letterSpacing:3, textTransform:"uppercase", margin:"0 0 8px" }}>Community</p>
        <h1 style={{ fontFamily:FONTS.display, fontWeight:900, fontSize: isMobile ? 32 : 44, textTransform:"uppercase", color:B.black, margin:0, lineHeight:0.95, letterSpacing:"-0.5px" }}>Inner Circle</h1>
      </div>
 ...
 {communityInviteSent ? "Sent Done" : communityInviting ? "Sending…" : "Invite"}
 </button>
 </div>
...
.then(({ error }) => {
 ...
 <div style={{ background: B.black, padding: "10px 14px", marginBottom: 10, display: "flex", justifyContent: "space-between", alignItems: "center" }}><div><div style={{ fontSize: 10, color: B.ivory, fontWeight: 700, letterSpacing: 1.5, textTransform: "uppercase", marginBottom: 2 }}>{lead.slot.date}</div><div style={{ fontSize: 18, color: B.ivory, fontWeight: 900, fontFamily: FONTS.display, letterSpacing: "-0.5px", lineHeight: 1 }}>{lead.slot.time}</div></div>{(() => { if (!lead.scheduledAt) return null; const days = Math.ceil((new Date(lead.scheduledAt) - Date.now()) / (1000 * 60 * 60 * 24)); if (days < 0) return <div style={{ fontSize: 8, color: B.silver, fontWeight: 700, letterSpacing: 1, textTransform: "uppercase" }}>Past</div>; if (days === 0) return <div style={{ fontSize: 9, color: B.white, fontWeight: 700, letterSpacing: 1, textTransform: "uppercase", background: B.blush, padding: "4px 10px" }}>Today</div>; if (days === 1) return <div style={{ fontSize: 9, color: B.white, fontWeight: 700, letterSpacing: 1, textTransform: "uppercase", background: B.blush, padding: "4px 10px" }}>Tomorrow</div>; return <div style={{ fontSize: 9, color: B.ivory, fontWeight: 700, letterSpacing: 1, textTransform: "uppercase", opacity: 0.8 }}>In {days} days</div>; })()}</div>
 ...
 {resourceForm.file && <div style={{ fontSize:10, color:B.success, marginTop:6 }}>Done {resourceForm.file.name}</div>}
 </div>
...
.then(({ data }) => {
 ...
 <span style={{ fontSize:7, background:"#2D7D4E", color:B.white, padding:"2px 6px", fontWeight:700, letterSpacing:1 }}>GRAD GRAD</span>
 </div>
...
.then(() => {
 ...
 body: { action: 'log_visit', referrer_code: ref }
 }).then(({ data }) => {
 if (data?.referral_id) {
 try { sessionStorage.setItem('sjtn_ref_id', data.referral_id); } catch {}
 }
 });
 window.history.replaceState({}, '', window.location.pathname);
 }
 ...
 {screen === "community" && activeUser && sessionValid && <CommunityPortal user={activeUser} onLogout={handleLogout} onUpgrade={() => { window.open("https://calendly.com/sayjesstonails-info/free-discovery-call?name=" + encodeURIComponent(activeUser.firstName || "") + "&email=" + encodeURIComponent(activeUser.email || ""), "_blank"); }} />}
